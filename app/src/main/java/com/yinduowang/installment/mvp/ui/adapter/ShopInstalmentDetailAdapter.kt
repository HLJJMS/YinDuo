...
import com.yinduowang.installment.mvp.model.entity.ShopBillDetailListBean
import com.yinduowang.installment.mvp.model.entity.ShopStageDetailsPlanlistBean

/**
 * Description：商城账单分期详情
 * Author：田羽衡
 ...
 * Deprecated： false
 */
class ShopInstalmentDetailAdapter(data: List<MultiItemEntity>?) : BaseMultiItemQuickAdapter<MultiItemEntity, BaseViewHolder>(data) {
    companion object {
    ...
    override fun convert(helper: BaseViewHolder, item: MultiItemEntity?) {
        when (helper?.itemViewType) {
            TYPE_LEVEL_0 -> {
                val father = item as ShopStageDetailsPlanlistBean
            ...
            TYPE_LEVEL_1 -> {
                val son = item as ShopBillDetailListBean
...
//                循环判断最上边的一个父类repaymentState的值
                for (index in helper.adapterPosition downTo 0) {
                    if (mData.get(index).itemType == 0) {
                        val father = mData.get(index) as ShopStageDetailsPlanlistBean
