...
class DialogLoanQuickAdapter(data: List<MultiItemEntity>?) : BaseMultiItemQuickAdapter<MultiItemEntity, BaseViewHolder>(data) {
    companion object {
    ...
    override fun convert(helper: BaseViewHolder, item: MultiItemEntity?) {
        when (helper.itemViewType) {
            TYPE_LEVEL_0 -> {
                val budgetMoneyInfo = item as BudgetMoneyInfo
            ...
            TYPE_LEVEL_1 -> {
                val budgetInfo = item as BudgetInfo
